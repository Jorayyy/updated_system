...
<head>
    ...
    <title>DTR - All Employees - {{ date('F Y', mktime(0, 0, 0, $month, 1, $year)) }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="container {{ !$loop->last ? 'page-break' : '' }}">
        ...
        <div class="footer">
            Generated on {{ now()->format('F d, Y h:i A') }} | MEBS HIYAS Management System | Page {{ $index + 1 }} of {{ count($dtrs) }}
        </div>
    </div>
    @endforeach
</body>
