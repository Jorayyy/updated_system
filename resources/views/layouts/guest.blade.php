            ...
            <div class="hidden lg:flex lg:w-1/2 flex-col justify-center items-center p-12 relative overflow-hidden">
                ...
                <div class="text-center z-10" x-show="show" x-transition:enter="transition ease-out duration-1000" x-transition:enter-start="opacity-0 transform translate-y-10" x-transition:enter-end="opacity-100 transform translate-y-0">
                    ...
                    <div class="mb-8 group">
                        ...
                        @endphp
                        @if($companyLogo)
                            <img src="{{ asset('storage/' . $companyLogo) }}" alt="Company Logo" class="w-72 h-auto mx-auto drop-shadow-2xl transition-transform duration-500 group-hover:scale-105 object-contain">
                        @else
                            <!-- Text-based Logo -->
                            <div class="flex items-center justify-center mb-4 transition-all duration-500 group-hover:scale-110">
                                <span class="text-8xl font-black text-slate-800 tracking-tighter drop-shadow-sm" style="font-family: 'Times New Roman', Times, serif;">M</span>
                                <div class="relative mx-[-5px]"> {{-- Negative margin to simulate the overlap in the logo --}}
                                    <div class="w-16 h-16 rounded-full bg-blue-100 flex items-center justify-center border-2 border-blue-600 shadow-md relative">
                                        {{-- Yellow Glow/Circle background to match the logo image --}}
                                        <div class="absolute inset-0 rounded-full border-4 border-yellow-300 transform -rotate-12 scale-110"></div>
                                        <span class="text-4xl font-black text-slate-800 relative z-10" style="font-family: 'Times New Roman', Times, serif;">e</span>
                                    </div>
                                </div>
                                <span class="text-8xl font-black text-slate-800 tracking-tighter drop-shadow-sm" style="font-family: 'Times New Roman', Times, serif;">BS</span>
                            </div>
                        @endif
                    </div>
