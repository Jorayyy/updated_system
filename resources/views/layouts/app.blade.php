...
">
    ...
    <body class="font-sans antialiased bg-gray-100 transition-colors duration-200">
        <div class="min-h-screen flex">
            <!-- Left Sidebar -->
            <aside :class="sidebarOpen ? 'sidebar-expanded sidebar-open' : 'sidebar-collapsed'" class="fixed inset-y-0 left-0 bg-gray-800 text-white sidebar-transition z-30 flex flex-col overflow-visible">
                <!-- Logo Section with Notification Bell and Toggle -->
                <div class="h-16 flex items-center border-b border-gray-700 transition-all duration-300" :class="sidebarOpen ? 'justify-between px-4' : 'justify-center px-0'">
                    <div x-show="sidebarOpen" x-cloak class="flex items-center overflow-hidden gap-3">
                        <a href="{{ route('dashboard') }}" class="flex items-center flex-shrink-0 gap-3">
                            @php
                                $companyLogo = \App\Models\CompanySetting::getValue('company_logo');
                            @endphp
                            @if($companyLogo)
                                <img src="{{ asset('storage/' . $companyLogo) }}" alt="Logo" class="w-10 h-10 rounded-lg flex-shrink-0 object-contain">
                            @else
                                <div class="w-10 h-10 bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg flex items-center justify-center font-bold text-lg flex-shrink-0">
                                    {{ substr(\App\Models\CompanySetting::getValue('company_name', 'M'), 0, 1) }}
                                </div>
                            @endif
                            <span class="font-bold text-lg sidebar-text whitespace-nowrap sidebar-text-visible">
                                {{ \App\Models\CompanySetting::getValue('company_name', 'MEBS HIYAS') }}
                            </span>
                        </a>
                    </div>
                    <div class="flex items-center flex-shrink-0">
                        ...
                        <button @click="sidebarOpen = !sidebarOpen" 
                            class="rounded-lg hover:bg-gray-700 transition-all duration-300 toggle-btn flex items-center justify-center" 
                            :class="sidebarOpen ? 'p-2' : 'p-2'"
                            title="Toggle Sidebar">
                            <svg :class="!sidebarOpen ? 'rotate-180' : ''" class="w-5 h-5 transition-transform duration-300 text-gray-300 hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <nav id="sidebar-nav" class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                            ...
                            <a href="{{ route('leaves.manage') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition {{ request()->routeIs('leaves.manage') ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                                <div class="w-8 h-8 flex items-center justify-center flex-shrink-0 relative">
                                    ...
                                    @if(isset($pendingLeavesCount) && $pendingLeavesCount > 0)
                                        <span x-show="!sidebarOpen" class="absolute top-0 right-0 transform translate-x-1/2 -translate-y-1/2 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white ring-2 ring-gray-800">
                                            {{ $pendingLeavesCount }}
                                        </span>
                                    @endif
                                </div>
                                <div x-show="sidebarOpen" x-cloak class="flex-1 flex items-center justify-between sidebar-text" :class="sidebarOpen ? 'sidebar-text-visible' : 'sidebar-text-hidden'">
                    ...
                    @if(auth()->user()->isAccounting() || auth()->user()->isSuperAdmin())
                        ...
                        <!-- Salaries (Accounting Focus) -->
                        <div class="relative nav-item">
                            <a href="{{ route('accounting.salaries.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition {{ request()->routeIs('accounting.salaries.index') ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/>
                                </svg>
                                <span x-show="sidebarOpen" x-cloak class="sidebar-text" :class="sidebarOpen ? 'sidebar-text-visible' : 'sidebar-text-hidden'">Salary Mgmt</span>
                            </a>
                            <div x-show="!sidebarOpen" class="tooltip">Salary Management</div>
                        </div>

                        <!-- Bulk Adjustments -->
                        <div class="relative nav-item">
                            <a href="{{ route('accounting.adjustments.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition {{ request()->routeIs('accounting.adjustments.index') ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 11l3-3m0 0l3 3m-3-3v8m0-13a9 9 0 110 18 9 9 0 010-18z"/>
                                </svg>
                                <span x-show="sidebarOpen" x-cloak class="sidebar-text" :class="sidebarOpen ? 'sidebar-text-visible' : 'sidebar-text-hidden'">Bulk Adjustments</span>
                            </a>
                            <div x-show="!sidebarOpen" class="tooltip">Bulk Adjustments</div>
                        </div>
                        @endif
                        ...
                        <div class="relative nav-item">
                            ...
                            <div class="relative nav-item">
                                <a href="{{ route('designations.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition {{ request()->routeIs('designations.*') ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
                                    </svg>
                                    <span x-show="sidebarOpen" x-cloak class="sidebar-text" :class="sidebarOpen ? 'sidebar-text-visible' : 'sidebar-text-hidden'">Job Roles</span>
                                </a>
                                <div x-show="!sidebarOpen" class="tooltip">Job Roles</div>
                            </div>
                            ...
                            <!-- Campaigns -->
                            <div class="relative nav-item">
                                <a href="{{ route('campaigns.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition {{ request()->routeIs('campaigns.*') ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/>
                                    </svg>
                                    <span x-show="sidebarOpen" x-cloak class="sidebar-text" :class="sidebarOpen ? 'sidebar-text-visible' : 'sidebar-text-hidden'">Campaigns</span>
                                </a>
                                <div x-show="!sidebarOpen" class="tooltip">Campaigns</div>
                            </div>
