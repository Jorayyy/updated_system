        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex flex-wrap gap-2">
                <a href="{{ route('notifications.index') }}" 
                   class="inline-flex items-center px-4 py-2 rounded-xl text-sm transition-all {{ request('filter') === null ? 'bg-indigo-600 text-white shadow-md' : 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50' }}">
                    All Notifications ({{ auth()->user()->notifications()->count() }})
                </a>
                <a href="{{ route('notifications.index', ['filter' => 'unread']) }}" 
                   class="inline-flex items-center px-4 py-2 rounded-xl text-sm transition-all {{ request('filter') === 'unread' ? 'bg-indigo-600 text-white shadow-md' : 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50' }}">
                    Unread Only ({{ auth()->user()->unreadNotifications->count() }})
                </a>
            </div>
