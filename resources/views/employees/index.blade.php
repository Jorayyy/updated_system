            ...
            <div x-show="selectedEmployees.length > 0" x-cloak
                ...
                <div class="flex items-center gap-4">
                    ...
                    <form action="{{ route('employees.bulk-assign-site') }}" method="POST" class="flex items-center gap-2">
                        ...
                        <select name="site_id" required class="text-xs border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500">
                            <option value="">Move Site...</option>
                            @foreach($allSites as $site)
                                <option value="{{ $site->id }}">{{ $site->name }}</option>
                            @endforeach
                        </select>
                        <button type="submit" class="bg-indigo-600 text-white px-2 py-1.5 rounded text-xs hover:bg-indigo-700">Go</button>
                    </form>

                    <!-- Bulk Campaign Assign -->
                    <form action="{{ route('employees.bulk-assign-campaign') }}" method="POST" class="flex items-center gap-2 border-l pl-4 border-indigo-200">
                        @csrf
                        <template x-for="id in selectedEmployees">
                            <input type="hidden" name="employee_ids[]" :value="id">
                        </template>
                        <select name="campaign_id" required class="text-xs border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 font-bold text-blue-600 uppercase">
                            <option value="">Move Campaign...</option>
                            @foreach($allCampaigns as $campaign)
                                <option value="{{ $campaign->id }}">{{ $campaign->name }}</option>
                            @endforeach
                        </select>
                        <button type="submit" class="bg-blue-600 text-white px-2 py-1.5 rounded text-xs hover:bg-blue-700 font-bold">Assign</button>
                    </form>

                    <!-- Bulk Designation Assign -->
                    <form action="{{ route('employees.bulk-assign-designation') }}" method="POST" class="flex items-center gap-2 border-l pl-4 border-indigo-200">
                        @csrf
                        <template x-for="id in selectedEmployees">
                            <input type="hidden" name="employee_ids[]" :value="id">
                        </template>
                        <select name="designation_id" required class="text-xs border-gray-300 rounded-md focus:ring-green-500 focus:border-green-500 font-bold text-green-600 uppercase">
                            <option value="">Move Job Role...</option>
                            @foreach($allDesignations as $designation)
                                <option value="{{ $designation->id }}">{{ $designation->name }}</option>
                            @endforeach
                        </select>
                        <button type="submit" class="bg-green-600 text-white px-2 py-1.5 rounded text-xs hover:bg-green-700 font-bold">Assign</button>
                    </form>
                </div>
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6 transition-colors duration-200">
                <div class="p-6">
                    <form method="GET" class="flex flex-wrap items-center gap-4">
                        ...
                        <select name="designation_id" class="border-gray-300 rounded-md shadow-sm text-sm font-bold text-green-600">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Job Role</th>
                                ...
                                @forelse($employees as $employee)
                                    <tr class="transition-colors duration-200" :class="selectedEmployees.includes('{{ $employee->id }}') ? 'bg-indigo-50' : ''">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            <div class="font-bold text-blue-600 uppercase">{{ $employee->campaign?->name ?? 'N/A' }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            <div class="font-bold text-green-600 uppercase">{{ $employee->designation?->name ?? 'N/A' }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            <div class="text-xs text-gray-700">{{ $employee->site?->name ?? 'N/A' }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <div class="flex flex-col gap-1">
                                                <span class="px-2 py-0.5 text-[10px] rounded-full font-bold uppercase tracking-wider w-fit
