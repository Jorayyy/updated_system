<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Add New Job Role / Designation') }}
        </h2>
    </x-slot>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name') }}" required
                                    placeholder="e.g. Lead Developer, QA Analyst"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                @error('name') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                            </div>

                            <div>
                                <label for="hierarchy_level" class="block text-sm font-medium text-gray-700">Hierarchy / Priority Level (6-100) *</label>
                                <input type="number" name="hierarchy_level" id="hierarchy_level" value="{{ old('hierarchy_level', 10) }}" required min="6" max="100"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <p class="mt-1 text-xs text-gray-500 italic">Levels 6-100 are for custom roles. Levels 1-5 are reserved for system campaigns.</p>
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-xs text-gray-500 italic">Determines the core permissions for any user assigned to this designation.</p>
