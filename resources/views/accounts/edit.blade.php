<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ $type == 'role' ? __('Edit User Role') : __('Edit Campaign Account') }} - {{ $account->name }}
        </h2>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('accounts.update', $account) }}">
                        ...
                        <div class="space-y-4">
                            <div>
                                <label for="name" class="block text-sm font-bold text-gray-700 uppercase tracking-tight mb-1">
                                    {{ $type == 'role' ? 'Role Name *' : 'Campaign Name *' }}
                                </label>
                            ...
                            @if($type == 'role')
                                <div>
                                    ...
                                    <p class="mt-1 text-[10px] text-gray-400 italic font-medium uppercase tracking-tighter">
                                        Prevents lower levels from editing higher ones.
                                    </p>
                                ...
                                <div>
                                    ...
                                    <select name="system_role" id="system_role" required
                                        class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition">
                                        <option value="employee" {{ old('system_role', $account->system_role) == 'employee' ? 'selected' : '' }}>Employee (Standard Personnel)</option>
                                        <option value="accounting" {{ old('system_role', $account->system_role) == 'accounting' ? 'selected' : '' }}>Accounting (Payroll & Finance)</option>
                                        <option value="hr" {{ old('system_role', $account->system_role) == 'hr' ? 'selected' : '' }}>HR (People Management)</option>
                                        <option value="admin" {{ old('system_role', $account->system_role) == 'admin' ? 'selected' : '' }}>Admin (Full Site Control)</option>
                            ...
                            @else
                                <input type="hidden" name="hierarchy_level" value="{{ $account->hierarchy_level }}">
                                <input type="hidden" name="system_role" value="{{ $account->system_role }}">
                            @endif

                            <div>
                                <label class="flex items-center gap-3 cursor-pointer group">
                                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $account->is_active) ? 'checked' : '' }} class="w-5 h-5 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 transition">
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="3"
                                    class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition">{{ old('description', $account->description) }}</textarea>
                        ...
                        <div class="mt-8 flex justify-end gap-3 pt-4 border-t border-gray-100">
                            ...
                            <button type="submit" class="bg-indigo-600 text-white px-8 py-2.5 rounded-lg hover:bg-indigo-700 font-bold shadow-lg hover:shadow-xl uppercase text-xs tracking-widest transition transform hover:-translate-y-0.5">
                                {{ $type == 'role' ? 'Update User Role' : 'Update Campaign' }}
                            </button>
                        </div>
                    </form>
                </div>
