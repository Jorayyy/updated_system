<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            <a href="{{ route('computers.index') }}" class="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <form method="POST" action="{{ route('computers.update', $computer) }}">
                ...
                <div class="space-y-6">
                    <!-- PC Number -->
                    <div>
                        <label for="pc_number" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            PC Number <span class="text-red-500">*</span>
                        </label>
                        <input type="text" name="pc_number" id="pc_number" value="{{ old('pc_number', $computer->pc_number) }}" 
                            class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white @error('pc_number') border-red-500 @enderror"
                            required>
                        @error('pc_number')
                    ...
                    <div>
                        ...
                        <select name="status" id="status" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white" required>
                    ...
                    <!-- Specs -->
                    <div>
                        <label for="specs" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            Specifications
                        </label>
                        <textarea name="specs" id="specs" rows="3" 
                            class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">{{ old('specs', $computer->specs) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="notes" id="notes" rows="2" 
                            class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">{{ old('notes', $computer->notes) }}</textarea>
                    </div>
