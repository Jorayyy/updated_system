                ...
                <div class="p-6">
                    <form method="GET" action="{{ route('reports.employees') }}" class="flex flex-wrap gap-4 items-end">
                        <div>
                            ...
                            <select id="campaign_id" name="campaign_id" class="mt-1 block border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                <option value="">All Campaigns</option>
                                @foreach($campaigns as $camp)
                                    <option value="{{ $camp->id }}" {{ $campaignId == $camp->id ? 'selected' : '' }}>{{ $camp->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($employees as $employee)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $employee->campaign?->name ?? 'N/A' }}</td>
