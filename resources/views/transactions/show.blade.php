    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex items-center">
                <a href="{{ url()->previous() }}" class="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 mr-3">
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Employee Info -->
                        <div>
                            <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Employee</h4>
                            <p class="text-gray-900 dark:text-white">{{ $transaction->user->name }}</p>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $transaction->user->employee_id }}</p>
                        </div>

                        <!-- Dates -->
                        @if($transaction->effective_date)
                            <div>
                                <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Effective Date</h4>
                                ...
                                @if($transaction->days_count)
                                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ $transaction->days_count }} day(s)</p>
                                @endif
                            </div>
                        @endif
                        ...
                        @if($transaction->time_from || $transaction->time_to)
                            <div>
                                <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Time</h4>
                                <p class="text-gray-900 dark:text-white">
                        ...
                        @if($transaction->leaveType)
                            <div>
                                <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Leave Type</h4>
                                <p class="text-gray-900 dark:text-white">{{ $transaction->leaveType->name }}</p>
                            </div>
                        @endif
                    </div>
                    ...
                    <!-- Reason -->
                    <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                        <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Reason / Details</h4>
                        <p class="text-gray-900 dark:text-white whitespace-pre-wrap">{{ $transaction->reason }}</p>
                    </div>
                    ...
                    @if($transaction->isRejected() && $transaction->rejection_reason)
                        <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                            <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
                                ...
                                <p class="text-red-700 dark:text-red-400">{{ $transaction->rejection_reason }}</p>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <h4 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Approval Timeline</h4>
                    ...
                    <div class="relative">
                        ...
                        <div class="relative flex items-start mb-6">
                            ...
                            <div class="ml-12">
                                <p class="font-medium text-gray-900 dark:text-white">Request Filed</p>
                                <p class="text-sm text-gray-500 dark:text-gray-400">{{ $transaction->created_at->format('M d, Y h:i A') }}</p>
                            </div>
                        </div>
                        ...
                        <div class="relative flex items-start mb-6">
                            <div class="absolute left-0 w-8 h-8 rounded-full 
                                {{ $transaction->hr_approved_at ? 'bg-green-500' : ($transaction->isRejected() ? 'bg-red-500' : 'bg-gray-300 dark:bg-gray-600') }} 
                            ...
                            <div class="ml-12">
                                <p class="font-medium text-gray-900 dark:text-white">HR Approval</p>
                                @if($transaction->hr_approved_at)
                                ...
                                @else
                                    <p class="text-sm text-yellow-600 dark:text-yellow-400">Awaiting HR approval</p>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="relative flex items-start">
                            <div class="absolute left-0 w-8 h-8 rounded-full 
                                {{ $transaction->admin_approved_at ? 'bg-green-500' : ($transaction->isRejected() ? 'bg-red-500' : 'bg-gray-300 dark:bg-gray-600') }} 
                                flex items-center justify-center">
                            ...
                            <div class="ml-12">
                                <p class="font-medium text-gray-900 dark:text-white">Admin Approval (Final)</p>
                                @if($transaction->admin_approved_at)
                                    <p class="text-sm text-green-600 dark:text-green-400">
                                ...
                                @elseif($transaction->status === 'hr_approved')
                                    <p class="text-sm text-yellow-600 dark:text-yellow-400">Awaiting Admin approval</p>
                                @else
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Pending HR approval first</p>
                                @endif
                            </div>
                        </div>
                    </div>
