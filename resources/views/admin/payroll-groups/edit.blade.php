                    ...
                    <form method="POST" action="{{ route('payroll-groups.update', $payrollGroup) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input id="name" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" 
                                   type="text" name="name" value="{{ old('name', $payrollGroup->name) }}" required autofocus />
                            @error('name')
                        ...
                        <div class="mb-4">
                            ...
                            <select id="period_type" name="period_type" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                <option value="weekly" {{ old('period_type', $payrollGroup->period_type) == 'weekly' ? 'selected' : '' }}>Weekly</option>
                                <option value="semi_monthly" {{ old('period_type', $payrollGroup->period_type) == 'semi_monthly' ? 'selected' : '' }}>Semi-Monthly</option>
                                <option value="monthly" {{ old('period_type', $payrollGroup->period_type) == 'monthly' ? 'selected' : '' }}>Monthly</option>
                            </select>
                            @error('period_type')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="block mt-4 mb-4">
                            <label for="is_active" class="inline-flex items-center">
                                <input id="is_active" type="checkbox" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" 
                                       name="is_active" value="1" {{ old('is_active', $payrollGroup->is_active) ? 'checked' : '' }}>
