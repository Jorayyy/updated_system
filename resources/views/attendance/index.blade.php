            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    ...
                    @if($status['status'] === 'completed' || $status['status'] === 'not_started')
                        ...
                        <div class="mt-8 pt-6 border-t border-gray-100" x-data="{ showSpecialForm: false }">
                            <div class="flex items-center justify-between">
                                <h4 class="text-md font-bold text-gray-800 flex items-center gap-2">
                                    ...
                                    Special / Weekend Duty
                                </h4>
                                <button @click="showSpecialForm = !showSpecialForm" class="text-sm font-medium text-indigo-600 hover:text-indigo-800">
                                    <span x-show="!showSpecialForm">Report Duty +</span>
                                    <span x-show="showSpecialForm">Cancel ×</span>
                                </button>
                            </div>
                            
                            <div x-show="showSpecialForm" x-transition class="mt-4 p-4 bg-indigo-50 rounded-xl border border-indigo-100 shadow-sm">
                                <p class="text-sm text-indigo-700 mb-4">Use this to record attendance for weekend shifts or special assignments that require proof of work.</p>
                                
                                <form action="{{ route('attendance.weekend-duty') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                                    @csrf
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Reason for Duty</label>
                                        <textarea name="reason" rows="2" required 
                                            class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm"
                                            placeholder="Write a brief description of the work performed..."></textarea>
                                    </div>
                                    
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Proof of Work (Photo/Screenshot)</label>
                                    ...
                                    <div class="flex justify-end">
                                        <button type="submit" class="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-bold text-sm shadow-md transition-all">
                                            Submit Proof & Clock In
                                        </button>
                                    </div>
                                </form>
                            </div>
