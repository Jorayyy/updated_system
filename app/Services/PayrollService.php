...
class PayrollService
{
    ...
    public function computePayroll(User $user, PayrollPeriod $period): Payroll
    {
        ...
        // Fetch Manual Adjustments (Human Intervention)
        $bonusAdjustments = BonusAdjustment::where('user_id', $user->id)
            ->where('is_processed', false)
            ->whereBetween('effective_date', [$period->start_date, $period->end_date])
            ->get();

        $manualBonuses = $bonusAdjustments->sum('amount');

        $deductionAdjustments = DeductionAdjustment::where('user_id', $user->id)
            ->where('is_processed', false)
            ->whereBetween('effective_date', [$period->start_date, $period->end_date])
            ->get();

        $manualDeductions = $deductionAdjustments->sum('amount');

        // Calculate net pay
        $grossPay = $earnings['basic_pay'] + $earnings['overtime_pay'] + 
                   $earnings['holiday_pay'] + $earnings['allowances'] + $manualBonuses;
        ...
        $totalDeductions = $deductions['sss'] + $deductions['philhealth'] + 
                          ...
                          $deductions['absent'] + $deductions['other'] + $manualDeductions;
        ...
        $payroll = Payroll::updateOrCreate(
            ...
            [
                ...
                'other_deductions' => $deductions['other'] + $manualDeductions,
        ...
        if ($payroll) {
            foreach ($bonusAdjustments as $adj) {
                $adj->update(['is_processed' => true, 'payroll_period_id' => $period->id]);
            }
            foreach ($deductionAdjustments as $adj) {
                $adj->update(['is_processed' => true, 'payroll_period_id' => $period->id]);
            }
        }
    ...
    public function processPayrollPeriod(PayrollPeriod $period): array
    {
        ...
        if ($period->site_id) {
            $query->where('site_id', $period->site_id);
        }

        // Apply account filter if set
        if ($period->account_id) {
            $query->where('account_id', $period->account_id);
        }
