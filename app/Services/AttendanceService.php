...
class AttendanceService
{
    ...
    /**
     * Record special duty (weekend/overtime) with proof
     */
    public function recordDutyWithProof(User $user, $imageFile, string $reason): array
    {
        // Check IP restriction
        $ipCheck = $this->checkIpRestriction();
        if (!$ipCheck['allowed']) {
        ...
        // 1. Save the proof image
        $path = $imageFile->store('attendance-proofs', 'public');

        DB::beginTransaction();
        try {
            // 2. Check if there's already an attendance record for today
            // If it's a weekend, it's fine to create a new one.
            $attendance = Attendance::where('user_id', $user->id)
                ->whereDate('date', $today)
                ->first();

            if ($attendance && $attendance->hasTimedIn()) {
                // If already clocked in, update the existing record with proof
                $attendance->proof_image = $path;
                $attendance->proof_reason = $reason;
                $attendance->save();
                
                $message = 'Proof of duty uploaded successfully for today\'s shift.';
            } else {
                // Create new attendance record for special duty
                $attendance = Attendance::create([
                    ...
                    'proof_image' => $path,
                    'proof_reason' => $reason,
                    'remarks' => 'Special/Weekend Duty: ' . $reason
                ]);
                
                $message = 'Special Duty / Weekend Clock In recorded at ' . $now->format('h:i A');
            }

            AuditLog::log(
                'special_duty_clock_in',
                Attendance::class,
                $attendance->id,
                null,
                ['time_in' => $now->toDateTimeString(), 'proof' => $path],
                "Employee {$user->name} recorded special duty with proof"
            );
            ...
            return [
                'success' => false,
                'message' => 'Failed to record duty: ' . $e->getMessage(),
            ];
        }
    }
