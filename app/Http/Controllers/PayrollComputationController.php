...
class PayrollComputationController extends Controller
{
    ...
    {
        $query = Payroll::with(['user', 'user.campaign', 'user.account'])
        ...
        $payrolls = $query->orderBy('user_id')
            ->paginate(20);

        $summary = $this->computationService->getPeriodSummary($period, $request->only(['campaign_id', 'account_id']));

        // Get status counts
        $statusCounts = Payroll::where('payroll_period_id', $period->id);

        if ($request->filled('campaign_id')) {
            $statusCounts->whereHas('user', function ($q) use ($request) {
                $q->where('campaign_id', $request->campaign_id);
            });
        }
        ...
        $allCampaigns = \App\Models\Campaign::where('is_active', true)->get();
        $allAccounts = \App\Models\Account::where('is_active', true)->get();

        return view('payroll.computation.show', compact('period', 'payrolls', 'summary', 'statusCounts', 'allCampaigns', 'allAccounts'));
    }
