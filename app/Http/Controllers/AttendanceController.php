...
class AttendanceController extends Controller
{
    ...
    public function manage(Request $request)
    {
        ...
        $campaignFilter = $request->get('campaign_id');
        $designationFilter = $request->get('designation_id');
        $statusFilter = $request->get('status');
        $searchTerm = $request->get('search');

        $query = Attendance::with(['user.designation', 'user.campaign', 'user.site'])
            ->whereDate('date', $date);
        ...
        if ($campaignFilter) {
            $query->whereHas('user', function ($q) use ($campaignFilter) {
                $q->where('campaign_id', $campaignFilter);
            });
        }

        if ($designationFilter) {
            $query->whereHas('user', function ($q) use ($designationFilter) {
                $q->where('designation_id', $designationFilter);
            });
        }
    ...
    /**
     * Record weekend duty or overtime with proof
     */
    public function recordWeekendDuty(Request $request)
    {
        $request->validate([
        ...
        // Use the service to record duty with proof
        $result = $this->attendanceService->recordDutyWithProof(
            $user, 
