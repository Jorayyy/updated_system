...
class LeaveCreditsController extends Controller
{
    ...
    public function edit(User $employee, Request $request)
    {
        if (!auth()->user()->isSuperAdmin()) {
            return redirect()->route('leave-credits.index')->with('error', 'Unauthorized. Only Super Admin(s) can edit leave credits.');
        }
    ...
    {
        if (!auth()->user()->isSuperAdmin()) {
            return redirect()->route('leave-credits.index')->with('error', 'Unauthorized. Only Super Admin(s) can update leave credits.');
        }
