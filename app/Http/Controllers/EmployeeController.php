...
class EmployeeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $allAccounts = Account::where('is_active', true)->where('hierarchy_level', '>', 5)->get();
        $allDesignations = Designation::where('is_active', true)->orderBy('name')->get();

        return view('employees.index', compact('employees', 'departments', 'allSites', 'allCampaigns', 'allAccounts', 'allDesignations'));
    }
    ...
    public function create()
    {
        ...
        $accounts = Account::where('is_active', true)->where('hierarchy_level', '>', 5)->get();
        $designations = Designation::where('is_active', true)->orderBy('name')->get();
        return view('employees.create', compact('sites', 'campaigns', 'accounts', 'designations'));
    }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'role' => 'required|in:employee,accounting,hr,admin,super_admin',
        ...
        if ($role === 'super_admin' && !auth()->user()->isSuperAdmin()) {
            return back()->with('error', 'Unauthorized. Only Super Admin(s) can assign Super Admin role.')->withInput();
        }
    ...
    public function edit(User $employee)
    {
        ...
        $accounts = Account::where('is_active', true)->where('hierarchy_level', '>', 5)->get();
        $designations = Designation::where('is_active', true)->orderBy('name')->get();
        return view('employees.edit', compact('employee', 'sites', 'campaigns', 'accounts', 'designations'));
    }
    ...
    /**
     * Bulk assign employees to a campaign
     */
    public function bulkAssignCampaign(Request $request)
    {
        ...
        return redirect()->route('employees.index')
            ->with('success', count($request->employee_ids) . ' employees assigned to campaign successfully.');
    }

    /**
     * Bulk assign employees to an account (User Role)
     */
    public function bulkAssignAccount(Request $request)
    {
        ...
        return redirect()->route('employees.index')
            ->with('success', count($request->employee_ids) . ' employees assigned to access level successfully.');
    }

    /**
     * Bulk assign employees to a designation (Job Role)
     */
    public function bulkAssignDesignation(Request $request)
    {
        $request->validate([
            'employee_ids' => 'required|array',
            'employee_ids.*' => 'exists:users,id',
        ...
        return redirect()->route('employees.index')
            ->with('success', count($request->employee_ids) . ' employees assigned to job role successfully.');
    }
}
